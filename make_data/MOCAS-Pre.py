...
def data(csv):
	...
	for i,line in enumerate(csv):
		if i >= 1:
			...
			modality1_data1.append(line[55].strip('[').rstrip(']').split(', '))
			modality2 = line[53].strip('[').rstrip(']').replace("\n", '').split(', ')
			modality2_data2 = list(filter(None, modality2))
			modality2_data1.append(modality2_data2)
			for b in range(12,18,1):
...
def pkl_make(modality11,modality21,modality31,modality41,modality51,label1,train_id,val_id,test_id,pkl,epoch):
	...
	modality1_train = np.array(modality11)[train_id].reshape(train_id.shape[0],1,18)
	modality1_val = np.array(modality11)[val_id].reshape(val_id.shape[0],1,18)
	modality1_test = np.array(modality11)[test_id].reshape(test_id.shape[0],1,18)

	modality2_train = np.array(modality21)[train_id].reshape(train_id.shape[0],1,256)
	modality2_val = np.array(modality21)[val_id].reshape(val_id.shape[0],1,256)
	modality2_test = np.array(modality21)[test_id].reshape(test_id.shape[0],1,256)

	modality3_train = np.array(modality31)[train_id].reshape(train_id.shape[0],6,128)
	modality3_val = np.array(modality31)[val_id].reshape(val_id.shape[0],6,128)
	modality3_test = np.array(modality31)[test_id].reshape(test_id.shape[0],6,128)
	...
	modality5_train = np.array(modality51)[train_id].reshape(train_id.shape[0],2,1)
	modality5_val = np.array(modality51)[val_id].reshape(val_id.shape[0],2,1)
	modality5_test = np.array(modality51)[test_id].reshape(test_id.shape[0],2,1)
...
if __name__ == '__main__':
	...
	txt = open('Preprocessed_MOCAS_list.txt','r')
	txt1 = txt.readlines()
